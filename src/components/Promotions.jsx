...
export default function Promotions({ onProductClick }) {
  ...
  const renderPromoCard = (promo) => {
    ...
    return (
      <div
        className={`relative rounded-3xl overflow-hidden bg-gradient-to-br ${promo.bgColor} text-white shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 cursor-pointer h-full min-h-96 flex flex-col justify-between`}
      >
        {/* Discount badge */}
        <div className="absolute top-4 right-4 bg-yellow-400 text-gray-900 font-black text-lg px-3 py-1 rounded-2xl shadow-md rotate-3 z-10">
          -{promo.discount}
        </div>
        ...
        <div className="absolute -bottom-4 -right-4 w-20 h-20 bg-white/5 rounded-full pointer-events-none" />
      </div>
    );
  };
